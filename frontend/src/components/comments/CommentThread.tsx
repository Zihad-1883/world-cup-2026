      ...
      <div className="space-y-6">
        {loading ? (
          <div className="py-12 flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest animate-pulse">Loading thoughts...</p>
          </div>
        ) : comments.length > 0 ? (
          comments.map((comment) => (
            <div key={comment.id} className="group relative flex space-x-4 animate-in slide-in-from-left-2 duration-300">
              <Avatar url={comment.author.avatarUrl} username={comment.author.username} size="md" />
              ...
              <div className="flex-1 space-y-2 min-w-0">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex space-x-2">
                      <button
                        ...
                        className="flex items-center space-x-1.5 bg-green-500 text-black text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl hover:bg-green-400 transition-all active:scale-95"
                      >
                        <Check className="h-3 w-3" /> <span>Save</span>
                      </button>
                      <button
                        onClick={cancelEdit}
                        className="flex items-center space-x-1.5 bg-white/5 text-slate-400 text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl hover:bg-white/10 transition-all active:scale-95"
                      >
                ...
                {replyingTo === comment.id && (
                  <div className="mt-3 ml-4 border-l-2 border-green-500/30 pl-4 space-y-2 animate-in slide-in-from-top-2 duration-200">
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white text-sm focus:outline-none focus:border-green-500 transition-all resize-none font-medium placeholder:text-slate-700"
                    />
                    <div className="flex space-x-2">
                      <button
                        onClick={() => handleReplySubmit(comment.id)}
                        disabled={replyLoading || !replyText.trim()}
                        className="flex items-center space-x-1.5 bg-green-500 text-black text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl hover:bg-green-400 disabled:opacity-50 transition-all active:scale-95"
                      >
                        {replyLoading ? <div className="h-3 w-3 border border-black/30 border-t-black rounded-full animate-spin" /> : <Reply className="h-3 w-3" />}
